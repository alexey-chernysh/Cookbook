...
public class MainDB {
    ...
    public static String getNameColumnHeader(Cursor cursor){
        String neutralNameHeader = App.getInstance().getResourceString(R.string.name_field);
        String nameHeader = neutralNameHeader
                          + "_"
                          + App.getInstance().getLanguaget();
        int nameIndex = cursor.getColumnIndex(nameHeader);
        if(nameIndex >= 0) return nameHeader;
        nameHeader = neutralNameHeader
                + "_"
                + App.getInstance().getResourceString(R.string.english_language);
    ...
    public static String getNameByKey(String tableName, int key) {
        ...
        String filter = App.getInstance().getResourceString(R.string.key_field)
                + App.getInstance().getResourceString(R.string.is_equal_to)
                + key;
    ...
    public static String getFilterEqualTo(int nameId, int value){
        return App.getInstance().getResourceString(nameId) + "==" + value;
    }
